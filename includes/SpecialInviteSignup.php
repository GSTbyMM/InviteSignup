...
class SpecialInviteSignup extends SpecialPage {
	...
	public function execute( $par ) {
		...
		$out->addHTML(
			...
			Html::element( 'th', null, $this->msg( 'is-tableth-expiry' )->text() ) .
			Html::element( 'th', null, '' ) .
			$this->getAddRow() .
			Html::closeElement( 'thead' )
		);
		foreach ( $invites as $hash => $invite ) {
		    $expiry = $invite['expiry'] ? $lang->timeanddate( $invite['expiry'] ) : '';
            $whenSort = [ 'data-sort-value' => $invite['when'] ];
            $when = $lang->timeanddate( $invite['when'] );
            $email = $invite['email'];
            ...
            $out->addHTML(
                ...
                Html::element( 'td',
                    ...
                    $invite['used'] ? $lang->timeanddate( $invite['used'] ) : ''
                ) .
                Html::element( 'td', null, $groups ) .
                Html::element( 'td', null, $expiry ) .
                Html::rawElement( 'td',
                    null,
	...
	protected function getAddRow() {
		...
		return Html::openElement( 'tr' ) .
			...
			Html::element( 'td', null, $lang->timeanddate( wfTimestamp(), $user ) ) .
